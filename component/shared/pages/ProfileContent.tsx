...
export default function ProfileContent() {
  ...
  const [paymentType, setPaymentType] = useState<"bank" | "ewallet" | null>(
    null,
  );
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (res.ok && data.success) {
          ...
          toast.success("Profil berhasil diperbarui, Juragan! 😎");
        ...
        toast.error("Terjadi kesalahan koneksi ke server Laravel.");
      }
    },
  });
    ...
    <div className="mx-auto flex max-w-[980px] flex-col gap-5">
      <UserSectionTitle
        ...
        action={
          <button
            ...
            className="glass-card inline-flex items-center gap-2 rounded-full border border-[#e6b3a8] bg-[#fff3ef] px-4 py-2 text-sm font-semibold text-[#c35f46] dark:text-[#f0b2a7]"
          >
            <Edit3 size={16} />
            Ubah Profil
          </button>
      ...
      <div className="grid gap-5 lg:grid-cols-[240px_1fr]">
        <div className="glass-card rounded-[20px] border-2 border-[#e3d0c9] p-5 shadow-[0_10px_24px_rgba(15,23,42,0.05)] dark:border-slate-700/80">
          <div className="flex flex-col items-center text-center">
            <Image
              ...
              className="mt-4"
            />
            <h3 className="mt-4 text-[21px] font-semibold text-slate-900 dark:text-slate-100">
              {user?.name || "Pengguna"}
            </h3>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">
              {user?.role === "owner" ? "Pemilik Kos" : "Penyewa"} • Anggota
              Aktif
            </p>
        ...
        <div className="space-y-4">
          <div>
            <h3 className="mb-3 text-[18px] font-semibold text-slate-900 dark:text-slate-100">
              Informasi Dasar
            </h3>
              ...
              <div className="space-y-4 p-6">
                {!paymentType ? (
                  <>
                    ...
                    <div className="pt-2">
                      <p className="text-xs text-slate-500 dark:text-slate-400">
                        Fitur ini belum terkoneksi ke backend, jadi data hanya
                        tampil di UI sampai endpoint tersedia.
                      </p>
                    </div>
                  ...
                  <form
                    onSubmit={async (e) => {
                      ...
                      const payload =
                        paymentType === "bank"
                          ? {
                              nama_metode: bankForm.bankName,
                              tipe: "bank",
                              nomor_rekening: bankForm.accountNumber,
                              atas_nama: bankForm.accountHolder,
                            }
                          : {
                              nama_metode: ewalletForm.ewallet_type,
                              tipe: "e_wallet",
                              nomor_rekening: ewalletForm.phoneNumber,
                              atas_nama: ewalletForm.accountHolder,
                            };
                  ...
                  >
                    ...
                    <div className="flex gap-3 pt-4">
                      ...
                      <button
                        type="submit"
                        className="flex-1 rounded-lg bg-[#c35f46] px-4 py-3 font-semibold text-white transition hover:bg-[#b8533d]"
                      >
                        Simpan
                      </button>
                    </div>

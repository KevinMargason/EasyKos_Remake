...
const fallbackThreads: Thread[] = [
  {
    ...
    messages: [
      ...
      {
        from: "user",
        text: "Rp 1,5 juta, Nak. Itu sudah termasuk banyak fasilitas.",
      },
    ],
  },
  {
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        {unread ? (
          <span className="absolute -left-6 top-0 h-4 w-4 rounded-full bg-[#e65b42]" />
        ) : null}
      </div>
...
function Bubble({ text, side }: { text: string; side: ChatSide }) {
  ...
  return (
    <div className={`flex ${isOwner ? "justify-end" : "justify-start"}`}>
      <div
        className={`max-w-[320px] rounded-[18px] px-5 py-4 text-[18px] shadow-[0_6px_14px_rgba(15,23,42,0.08)] sm:max-w-[380px] ${isOwner ? "bg-[#c86654] text-white" : "glass-card border border-slate-200 bg-white text-slate-800 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"}`}
      >
        {text}
      </div>
    </div>
  ...
  useEffect(() => {
    const loadChats = async () => {
      try {
        ...
        console.warn(
          "Failed to load chats from API, using fallback:",
          error.message,
        );
        ...
        <div className="border-b-2 border-[#e8d7d0] px-5 py-4 dark:border-slate-800">
          <div className="flex items-center gap-4">
            ...
            <div className="mb-4 min-w-0">
              <div className="text-[20px] font-semibold text-slate-900 dark:text-slate-100">
                {selected.name}
              </div>

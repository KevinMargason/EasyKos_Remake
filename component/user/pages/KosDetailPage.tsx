...
export default function KosDetailPage({
  ...
  const handlePrev = () => {
    setCurrentImageIndex(
      (prev) => (prev - 1 + kos.images.length) % kos.images.length,
    );
  };
  ...
  const kosPrice =
    kos.price ||
    (kos.harga
      ? `Rp ${Number(kos.harga).toLocaleString("id-ID")}`
      : "Harga belum tersedia");
  const resolvedOwnerName =
    kos.owner?.name ||
    kos.owner?.nama ||
    owner?.name ||
    "Nama pemilik belum tersedia";

  const generalFacilities = safeParse(
    kos.facilities?.umum?.length
      ? kos.facilities.umum
      : (kos as any).fasilitas_umum,
  );
  const roomFacilities = safeParse(
    kos.facilities?.kamar?.length
      ? kos.facilities.kamar
      : (kos as any).fasilitas_kamar,
  );

  const rawRules = kos.rules?.length ? kos.rules : (kos as any).peraturan;
  const rulesList = Array.isArray(rawRules)
    ? rawRules
    : typeof rawRules === "string"
      ? rawRules.split("\n").filter((r) => r.trim() !== "")
      : [];

  const parsedImages = safeParse(
    kos.images?.length ? kos.images : (kos as any).foto,
  );
  const finalImages =
    parsedImages.length > 0
      ? parsedImages.map((img: string) =>
          img.startsWith("http") || img.startsWith("/")
            ? img
            : `${process.env.NEXT_PUBLIC_API_URL?.replace("/api", "")}/storage/${img}`,
        )
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      ...
      <div className="relative h-96 w-full bg-slate-200 dark:bg-slate-800">
        <Image
          src={finalImages[currentImageIndex] || "/Asset/kamar/kamar1.svg"}
      ...
      <div className="space-y-6 px-6 py-8">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-bold text-[#c86654]">
              {kosPrice}
            </span>
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6 dark:border-slate-700 dark:bg-slate-800">
              ...
              <div className="grid grid-cols-3 gap-3">
                {generalFacilities.length > 0 ? (
                  generalFacilities.map((facility: string) => (
                    <div
                      key={facility}
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6 dark:border-slate-700 dark:bg-slate-800">
              ...
              <div className="grid grid-cols-3 gap-3">
                {roomFacilities.length > 0 ? (
                  roomFacilities.map((facility: string) => (
                    <div
                      key={facility}
                      className="flex h-24 w-full flex-col items-center justify-center gap-1.5 rounded-xl bg-white p-3 dark:bg-slate-700"
                    >
        ...
        <PaymentModal
          ...
          onConfirm={async (data) => {
            try {
              ...
              const roomsCheckResponse = await fetch(
                `/api/kos/${kos.id}/rooms`,
              );
              ...
              const bookedRooms =
                roomsData?.data?.filter(
                  (r: any) => r.status === "booked" || r.status === "occupied",
                )?.length || 0;
